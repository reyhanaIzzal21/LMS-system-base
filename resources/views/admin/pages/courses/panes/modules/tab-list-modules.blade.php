            ...
            @foreach ($modules as $module)
                <div class="border border-slate-200 rounded-xl p-4 flex justify-between items-start">
                    ...
                    <div class="flex gap-3">
                        <form method="POST" action="{{ route('modules.move-up', $module->id) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit"><i class="ti ti-arrow-up"></i></button>
                        </form>

                        <form method="POST" action="{{ route('modules.move-down', $module->id) }}">
                            @csrf
                            @method('PATCH')
...
@include('admin.pages.courses.panes.modules.scripts.list-module')
