        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                    ...
                    <div class="bg-slate-50 p-3 px-5 flex items-center justify-between border-t border-slate-100">
                        ...
                        <button type="button"
                            onclick="openConfirmModal(
                                '{{ route('programs.toggle-active', $program->id) }}',
                                '{{ $program->is_active ? 'Nonaktifkan Program?' : 'Aktifkan Program?' }}',
                                '{{ $program->is_active ? 'Apakah anda yakin ingin menonaktifkan program ini? Program tidak akan tampil di halaman user.' : 'Apakah anda yakin ingin mengaktifkan program ini? Program akan tampil dan dapat diakses oleh user.' }}',
                                {{ $program->is_active ? 0 : 1 }},
                                '{{ $program->is_active ? 'Ya, Nonaktifkan' : 'Ya, Aktifkan' }}',
                                '{{ $program->is_active ? 'bg-slate-600 hover:bg-slate-700 shadow-slate-500/20' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-500/20' }}'
                            )"
                        ...
                        <div class="relative">
                            <button onclick="toggleDropdown('dropdown-{{ $program->id }}')"
            ...
            @empty
                <div
                    ...
                    <p class="text-slate-500 mb-6 text-center max-w-sm">Anda belum membuat program apapun. Mulai buat
                        program pertama Anda!</p>
                    <a href="{{ route('programs.create') }}"
